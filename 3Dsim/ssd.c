...
void displayHelp() {
    ...
    printf("  -p  Specify parameter filename (default: default_param.txt)\n");
    printf("  -t  Specify trace filename (default: default_trace.txt)\n");
    printf("  -o  Specify output filename (default: default_output.txt)\n");
    printf("  -s  Specify statistic filename (default: default_statistic.txt)\n");
}
...
void main(int argc, char *argv[])
{
    ...
    strcpy(ssd->tracefilename, "exchange.ascii");
    strcpy(ssd->outputfilename, "exchange_statistic.dat");
    strcpy(ssd->statisticfilename, "exchange_TDA_ex.dat");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0) {
            ...
            strcpy(ssd->parameterfilename, argv[i + 1]);
        } else if (strcmp(argv[i], "-t") == 0 && i + 1 < argc) {
            strcpy(ssd->tracefilename, argv[i + 1]);
        } else if (strcmp(argv[i], "-o") == 0 && i + 1 < argc) {
            strcpy(ssd->outputfilename, argv[i + 1]);
        } else if (strcmp(argv[i], "-s") == 0 && i + 1 < argc) {
            strcpy(ssd->statisticfilename, argv[i + 1]);
        }
    }

	printf("tracefilename:%s \n", ssd->tracefilename);
	printf("parameterfilename:%s      .begin simulate-------------------------\n", ssd->parameterfilename);
	printf("tracefilename:%s        .begin simulate-------------------------\n", ssd->tracefilename);
	printf("outputfilename:%s       .begin simulate-------------------------\n", ssd->outputfilename);
	printf("trace of statistic filename:%s     ,begin simulate-------------------------\n", ssd->statisticfilename);
...
void trace_output(struct ssd_info* ssd){
	...
	while (req != NULL)
	{
		...
		else
		{
			...
			while (sub != NULL)
			{
				// printf("sub->complete_time: %16lld %16lld\n", sub->complete_time, req->time);
				// 子请求的第一个
				if (start_time == 0)
...
void statistic_output(struct ssd_info *ssd)
{
		...
		for (p = 0; p < ssd->parameter->chip_channel[i]; p++)
		{
			for (j = 0; j < ssd->parameter->die_chip; j++)
			{
				for (k = 0; k < ssd->parameter->plane_die; k++)
				{
					...
					fprintf(ssd->outputfile, "%3ld erase operations,", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].plane_erase_count);
					fprintf(ssd->outputfile, "%3ld read operations,", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].plane_read_count);
					fprintf(ssd->outputfile, "%3ld write operations,", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].plane_program_count);
					fprintf(ssd->outputfile, "%3ld pre_process write operations\n", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].pre_plane_write_count);
					
					fprintf(ssd->statisticfile, "the %d channel, %d chip, %d die, %d plane has : ", i, p, j, k);
					fprintf(ssd->statisticfile, "%3ld erase operations,", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].plane_erase_count);
					fprintf(ssd->statisticfile, "%3ld read operations,", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].plane_read_count);
					fprintf(ssd->statisticfile, "%3ld write operations,", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].plane_program_count);
					fprintf(ssd->statisticfile, "%3ld pre_process write operations\n", ssd->channel_head[i].chip_head[p].die_head[j].plane_head[k].pre_plane_write_count);
				}
			}
		}
	}
	...
	if (ssd->read_request_count != 0)
		fprintf(ssd->outputfile, "read request average response time: %16llu\n", ssd->read_avg / ssd->read_request_count);
	if (ssd->write_request_count != 0)
		fprintf(ssd->outputfile, "write request average response time: %16llu\n", ssd->write_avg / ssd->write_request_count);
	fprintf(ssd->outputfile, "\n");
	...
	fprintf(ssd->statisticfile, "\n");
	if (ssd->read_request_count != 0)
		fprintf(ssd->statisticfile, "read request average response time: %16llu\n", ssd->read_avg / ssd->read_request_count);
	if (ssd->write_request_count != 0)
		fprintf(ssd->statisticfile, "write request average response time: %16llu\n", ssd->write_avg / ssd->write_request_count);
	fprintf(ssd->statisticfile, "\n");
